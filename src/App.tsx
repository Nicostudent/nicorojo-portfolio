...
const App = () => {
  ...
  return (
    <div className="bg-hexagon_big bg-top-left-bottom-right bg-20% bg-no-repeat flex md:justify-center justify-start  items-center h-screen text-slate-700">
      {/* Left panel */}
      <div className="md:w-1/5 h-full border-r-2  border-slate-200 flex justify-center items-center">
        <ul>
          <li
      ...
      {/* Middle panel */}
      <div className="md:w-1/2 flex pl-2 relative h-full justify-center align-middle items-center">
        <h1
          className="flex flex-col md:text-6xl text-3xl text-center font-bold mb-4 absolute right-0 left-0 top-10 m-auto cursor-pointer w-fit"
          onClick={() => setHoveredItem(hoveredItem === 'image' ? 'home' : 'image')}
          onMouseLeave={() => handleItemHover("home")}
          // onMouseLeave={() => handleItemHover('home')}
        >
          Nicolás Rojo
          <span className="md:text-lg text-sm  text-center md:mt-2 text-slate-900 animate-bounce">
            Front-End Developer
          </span>
        </h1>

        {/* Display information based on hovered item */}
        <div className="md:w-3/4 text-sm md:text-lg flex justify-center items-center align-middle">
          <div
